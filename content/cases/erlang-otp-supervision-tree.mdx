...
flowchart TB
    subgraph OTP["OTP：已证实的监督路径"]
        P["父 supervisor"] --> S["当前 supervisor"]
        S --> A["child A"]
        S --> B["child B"]
        S --> C["child C"]
        C --> X{"exit reason + restart type"}
        X -->|"不恢复"| N["移出监督状态"]
        X -->|"需要恢复"| R{"intensity / period"}
        R -->|"预算内"| ST["按 strategy 恢复"]
        R -->|"超限"| SD["当前 supervisor shutdown"]
        SD --> P
    end

    subgraph AG["Agent：必须补充的语义"]
        F["失败观察"] --> K{"失败类别与外部事实"}
        K -->|"安全可重放"| B1{"次数 + 成本 + 风险预算"}
...
<details className="evidence-card">
  ...
  - [`restart/2` 及 strategy 1472–1562](https://github.com/erlang/otp/blob/295c7737d1b25a270e3fc6fa2b7ec7ad4779cf17/lib/stdlib/src/supervisor.erl#L1472-L1562)：先记账，超限后 shutdown，再区分一个、后缀或全部恢复。
  - [`terminate_children/2`、`shutdown/1` 1571–1657](https://github.com/erlang/otp/blob/295c7737d1b25a270e3fc6fa2b7ec7ad4779cf17/lib/stdlib/src/supervisor.erl#L1571-L1657) 与 [`add_restart/1` 2248–2290](https://github.com/erlang/otp/blob/295c7737d1b25a270e3fc6fa2b7ec7ad4779cf17/lib/stdlib/src/supervisor.erl#L2248-L2290)：关闭协议、单调时钟、窗口裁剪与 `intensity = 0`。
...
可重建运行契约至少应固定任务输入、模型参数、工具 schema、知识快照、权限、检查点、已完成步骤与外部资源 ID。敏感凭证只能通过短期引用重新获取，不能复制进升级证据包。
