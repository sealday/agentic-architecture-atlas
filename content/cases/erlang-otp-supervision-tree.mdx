...
flowchart TB
    subgraph OTP["OTP supervision tree：已证实机制"]
        ...
        X --> R{"strategy + intensity/period"}
        R -->|"预算内"| A1["one_for_one / one_for_all / rest_for_one"]
...
**四层决策顺序。** 一次退出不是直接进入某种 strategy。实际逻辑先找到退出的 child，再用 restart type 与退出原因决定是否需要重启；只有需要重启时才记入强度窗口，随后应用 `one_for_one`、`one_for_all` 或 `rest_for_one`。如果记账结果已超限，则不再进入局部策略，而是让当前 supervisor 退出。

| 决策层 | 输入 | OTP 动作 | Agent 迁移问题 |
| --- | --- | --- | --- |
| 1. 识别对象 | `Pid` | 找到对应 child spec | 哪个任务、Agent、工具调用或协作组失败？ |
| 2. 判断是否恢复 | exit reason + restart type | 重启、删除规格或触发自动关闭 | 这是 crash、质量失败、工具失败还是业务拒绝？ |
