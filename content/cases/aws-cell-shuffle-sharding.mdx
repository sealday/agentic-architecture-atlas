...
**安全与滥用。** 攻击者可能选择 tenant/resource key 寻找 overlap，或制造高成本 prompt 迫使 shard 内重试。placement 应使用服务端 secret 的 keyed hash，外部不可预测；seed rotation 需要双读/迁移 epoch。每 tenant 仍要有 rate/cost/tool policy，poison signature 隔离与 quarantine cell。shuffle sharding降低资源相遇概率，不替代认证、授权、输入验证、内容安全或 DDoS 防护。
