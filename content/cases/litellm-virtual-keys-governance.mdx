...
Master Key 是重要例外。固定认证路径匹配它后直接返回 `PROXY_ADMIN` 身份，不附带普通 Virtual Key 的模型、团队和 Key 预算上下文；该身份可以调用 LLM routes，官方预算文档还明确 proxy admin 不受 rate limits。把它只用于控制面而不进入推理面是部署建议，不是 LiteLLM 内建保证。
...
Master Key 应只进入受控管理面，保存在秘密管理器中，不分发给应用。LiteLLM 不会内建阻止它调用推理路由，也不会对 proxy admin 应用普通 rate limits；管理面网络隔离、最小管理员集合、轮换和告警都属于部署责任。
