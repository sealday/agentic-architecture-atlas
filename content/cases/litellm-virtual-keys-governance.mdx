...
![虚拟 Key 把身份、模型权限和预算放在路由之前](/img/illustrations/litellm-virtual-keys-governance-control-path.png)

图中的关键关系是策略先于路由，且供应商凭据留在 Proxy 一侧；它不表示所有请求都会通过，也不证明预算已被原子预留。下面的 Mermaid 图继续保留 Guardrail 时序、Master Key 分支与日志流等精确拓扑。
...
这里要检查的是：预算与 RPM/TPM 门挡住了哪些请求，又有哪些费用已经进入在途状态？

![预算与 RPM/TPM 在模型访问前执行，但不构成原子费用预留](/img/illustrations/litellm-virtual-keys-governance-budget-gates.png)
...
再比较两条凭据路径：调用方只持有 Virtual Key 时，哪些治理门仍在路径上；调用方直接持有 provider credential 时，又绕过了什么？

![受治理的 Proxy 路径与直连供应商凭据的绕过风险](/img/illustrations/litellm-virtual-keys-governance-governance-boundary.png)

Virtual Key 路径把权限、预算与供应商凭据保管集中到 Proxy，但这不等于上游供应商授权。直接凭据会绕过网关治理；轮换 Virtual Key、provider key、账号或 fallback 也不得被用来规避供应商撤权、限流、封禁或使用政策。
