...
先看一次请求在哪些位置改变控制所有者。重点检查客户端意图、网关策略与 provider 执行是否落在不同层，以及业务恢复为何没有被画成模型响应之后的自动步骤。

![区分客户端、网关路由与业务恢复责任层](/img/illustrations/kong-ai-gateway-routing-resilience-control-layers.png)

图中的结论是：网关可以选择 LLM target 并按失败分类调整选路，但不能替业务判断外部副作用是否安全重放。下面的 Mermaid 保留精确拓扑，并把 LLM 推理与 MCP/A2A 动作分开。
...
先检查一次上游失败如何进入分类、预算判断和下一 target，以及预算耗尽时是否有明确的失败出口。
...
路由韧性不能抹去这些边界。认证与 ACL 决定谁能调用，attempt 观测帮助核对重复费用；幂等键、状态查询与补偿流程才处理可能重复的业务副作用。
