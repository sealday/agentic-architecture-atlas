...
第一个决策是选择 API 抽象层。默认 OpenAI 格式让客户端与 provider API 解耦，便于跨 provider fallback；代价是只能使用网关支持映射的能力。native `llm_format` 保留供应商特性，却重新绑定对应 provider，迁移面和 fallback 兼容性都要单独验证。

第二个决策是把算法写成业务目标，而不是把“智能路由”当一个功能。权重轮询适合计划配额，一致性哈希适合会话或缓存粘性，least-connections 反映在途容量。最低延迟策略优化响应速度，最低用量策略优化 token 或成本，语义路由选择领域能力，priority 表达确定的服务层级。

这些目标可能冲突。最低延迟不等于最低成本，最低用量不等于最好质量，粘性也会牺牲全局均衡；生产配置应为主目标与降级目标分别设验收条件。

第三个决策是先分类再 retry。连接错误和读取响应头超时属于传输不确定性；429 常表示容量或配额；500/502/503/504 代表不同上游失败。把它们全部加入 failover 可能提高成功率，却会放大调用、费用和尾延迟。

第四个决策是将 priority 与断路器组合。priority 给出组间顺序，断路器把近期失败的 target 暂时移出候选集；代价是每个数据平面的观测窗口、失败阈值和恢复探测都需要验证。所有 target 同时不健康时，文档声明请求失败为 HTTP 500，而不是绕过健康状态继续发送。

第五个决策是为模型推理和工具执行建立不同重试入口。模型层可以接受受限的重复生成；工具层只有在端到端幂等键、状态查询或补偿协议存在时才能恢复。网关不应替业务拥有这项判断。

## 生产化分析

失败预算应同时限制尝试次数与墙钟时间。`retries`、连接/读/写 timeout、客户端 deadline 必须联合计算，否则多个慢 target 会把一次请求拖成长尾；流式响应已经向客户端发送字节后，还要验证是否具备再次尝试的合法边界。

断路器验收要验证 3.13+ 的具体计数语义。官方说明失败计数是总失败数而非连续失败数，timeout 窗口内夹着一次成功不会清零；只有最后失败后的 `fail_timeout` 已过，随后一次成功才会清零并恢复健康。阈值设计不能套用“成功一次立即闭合”的常见假设。

观测应能回答四个问题：客户端请求是什么、算法选择了哪个 target、每次尝试为何结束、最终返回和累计成本是什么。AI 日志可包含 provider、请求/响应模型、token、延迟与成本；启用 payload 日志还会把正文带入日志或 trace，因此需要脱敏、访问控制与保留策略。

指标默认不是免费的。官方 Prometheus 指南说明 AI metrics 默认关闭，以避免高基数与性能问题；启用需要 Prometheus 插件的 `ai_metrics` 和 AI Proxy Advanced 的 `log_statistics`。指标只能证明网关观察到的流量，provider 账单仍需独立对账。
...
MCP 工具应按副作用等级分组。纯查询可在确定未返回结果时有限重试；带幂等键的写入先查状态再重放；无法去重的支付、消息、资源创建等动作进入人工或补偿流程。ACL 解决谁能调用，不解决调用是否能重复。

A2A 生产链必须保留 task ID、context ID 与状态变化。AI A2A Proxy 3.14+ 是透明观测与控制层，不拥有任务状态；重试 `message/send` 前应先查询或关联既有 task，取消也必须由 Agent/业务端确认结果。
...
**证据边界说明**：`已证实事实` 来自 Kong 官方文档及其固定官方仓库；`个人分析` 用于解释重试不确定性、费用和外部副作用。本文没有虚构客户、事故、指标、源码实现或生产保证，也不把模型 failover 扩展成 MCP 工具或 A2A 任务的透明重放。
