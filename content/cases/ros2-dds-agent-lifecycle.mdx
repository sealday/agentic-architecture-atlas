...
| Managed Node | 外部管理者请求 configure/activate/deactivate/cleanup/shutdown；lifecycle transition callback 的异常或 `CallbackReturn::ERROR` 由状态机处理 | supervisor 在资源、策略和联锁校验后才激活 Agent | 普通业务 callback 异常自动触发生命周期转换，或 Active 自动等于有业务权限 |
...
7. **失效与恢复**：deadline missed、liveliness lost、QoS incompatible、普通 topic/service/action/timer callback 的超时或异常，以及设备告警，都只是应用与 supervisor 必须显式观测的策略输入；它们不会自动触发 Managed Node 生命周期转换。supervisor 需要按策略请求 deactivate、cleanup、重新 configure 或其他恢复。只有 lifecycle transition callback（`on_configure`、`on_activate`、`on_deactivate`、`on_cleanup`、`on_shutdown`、`on_error`）抛出异常或返回 `CallbackReturn::ERROR`，才由 Managed Node 状态机沿错误路径处理：前五类进入 ErrorProcessing；`on_error` 本身在 ErrorProcessing 中执行，其结果决定清理成功后回 Unconfigured，或清理失败后进入 Finalized。
...
| 生命周期授权 | 外部 lifecycle manager；`alive/configured/active/authorized` 四态分离；激活前校验策略与联锁 | lifecycle state/transition、policy version、authority lease、activation reason | 普通 callback 故障由 supervisor 显式请求恢复；lifecycle transition callback 的异常/`ERROR` 才沿 ErrorProcessing 路径处理 |
...
- [`lifecycle_node_interface.hpp`](https://github.com/ros2/rclcpp/blob/53cf81e42e6530c2ed7a23489d6640966dccd083/rclcpp_lifecycle/include/rclcpp_lifecycle/node_interfaces/lifecycle_node_interface.hpp)、[`lifecycle_node_interface_impl.cpp`](https://github.com/ros2/rclcpp/blob/53cf81e42e6530c2ed7a23489d6640966dccd083/rclcpp_lifecycle/src/lifecycle_node_interface_impl.cpp) 与 [`lifecycle_node.cpp`](https://github.com/ros2/rclcpp/blob/53cf81e42e6530c2ed7a23489d6640966dccd083/rclcpp_lifecycle/src/lifecycle_node.cpp)：Jazzy 的六个 lifecycle transition callback、异常到 `CallbackReturn::ERROR` 的映射，以及 transition API。
