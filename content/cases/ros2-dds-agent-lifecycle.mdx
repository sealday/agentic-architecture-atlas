...
official_sources:
  ...
  - https://github.com/ros2/rclcpp/blob/53cf81e42e6530c2ed7a23489d6640966dccd083/rclcpp_lifecycle/src/lifecycle_node_interface_impl.cpp
...
**已证实事实**：Topic 是异步发布/订阅数据流，可有多个发布者与订阅者；Service 面向短时请求响应，不提供取消；Action 面向长任务，具备 Goal、Feedback、Result 与 Cancel 接口。三者不是同一种传输方式的不同命名，而是不同的控制合同。
...
这张表的结论是：通信语义决定任务如何被表达，QoS 和 Executor 决定它可能如何到达与运行，Lifecycle 决定节点何时可处理；物理安全与业务授权仍需独立判定。

## 事实边界

这一节先划清版本、机制与跨域推断，避免把 ROS 2 通信能力扩大成物理安全保证。
...
阅读下图时先区分三种控制权。Supervisor 决定节点是否进入受管工作状态，Action server 决定长任务是否接纳和终止。Safety controller 可以绕过 Agent，直接使设备安全停机。
...
**说明性场景：移动机器人运送料箱到工作站。** 这不是生产事故复述，只用已证实机制说明控制权如何交接。
...
3. 激活成功后，激光雷达、位姿、设备健康与审计事件持续走 Topic。Agent 用 Service 查询地图或夹具配置，因为它需要一个有界的短答复；导航与取放则走 Action，因为任务要持有 Goal、反馈、结果与取消状态。

4. Action server 接纳“运送料箱”Goal 后周期发布位置和阶段反馈。执行循环检查资源预算、授权租约、联锁与取消状态，只有真实终态出现后才发布 Result。
...
源码只需要核对三个问题：Executor 如何看见待处理工作，Action 如何完成协作取消，Lifecycle 哪些错误会进入状态机。它不证明业务授权、调度实时性或设备已停。
...
QoS 的决策问题不是“可靠是否更好”，而是数据允许丢失、陈旧或晚到到什么程度，以及不兼容时系统是否能显式失败。
...
结论不是统一采用 Reliable。高频感知可允许新帧覆盖旧帧，低频控制状态则可能更重视交付；每类数据都要明确年龄、丢弃、连接失败与降级政策。
...
第二个决策是调度隔离。实时控制 callback 与模型推理 callback 不应共享无界执行队列；前者需要独立互斥组、Executor、受控 OS 调度、预分配内存和 WCET 测量，后者需要并发、CPU/GPU、token 与队列预算。

**个人分析**：需要确定处理顺序时，应评估显式 WaitSet 或更适合实时约束的执行方案。普通 Executor、Multi-Threaded 配置或 Reentrant 许可都不能单独支持 hard real-time 结论。

第三个决策是生命周期授权。外部 manager 可以请求 configure、activate、deactivate、cleanup 与 shutdown。进程 alive、节点 configured、Lifecycle Active 和业务 authorized 必须是四个不同判定。
...
生产检查应从故障后谁采取什么动作出发，而不是只看所有节点是否仍在运行。
...
**Executor 过载。** DDS 队列年龄、callback latency、drop、OOM、Action admission/reject 与 feedback lag 要共同观测。过载时采用有界 Keep Last、按数据类别丢弃、Service 限时、Action admission 与发布限速；禁止无界重试，并为安全 callback 保留预算。
...
**权限材料失效。** SROS2 keystore 把 identity 与 permission trust chain 分开。Enclave 带有私钥/证书、签名 permissions、domain governance 与 CA 材料；Enforce 策略可拒绝不允许的 Topic 发布或订阅。
...
1. 用 Topic、Service、Action 分开承载持续流、有界请求响应和长任务。
...
迁移条件是任务有可区分的状态、终态与控制所有者；否则换接口名称不会自动形成可靠合同。
...
停止条件是：只要错误重试、迟到命令或误判取消可能造成现实伤害，就必须先建立独立安全链、设备回执、授权门与明确恢复流程。
