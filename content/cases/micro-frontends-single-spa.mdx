...
**已证实事实**：single-spa 的 root config 注册应用、提供激活条件并启动生命周期。`activeWhen` 根据当前位置判断应用是否活跃；`start()` 之前只能加载，启动后才会 bootstrap、mount 或 unmount。独立流水线能否带来独立交付，取决于这些公开合同是否稳定，而不是注册了多少应用。
...
本案例钉扎 single-spa 仓库提交 [`297483c11265050542d7ef107d14c44936c51883`](https://github.com/single-spa/single-spa/tree/297483c11265050542d7ef107d14c44936c51883)，仓库 `package.json` 的版本为 6.0.3。该提交本身只更新资助配置，但它固定了本次分析使用的完整源码快照。
