---
title: Micro Frontends + single-spa：用垂直业务切片约束多 Agent 所有权
...
confidence: high
summary: 从运行时组合、独立部署和生命周期契约中分析 Agent 团队边界与共享 Shell。
...
official_sources:
  ...
  - https://github.com/single-spa/single-spa/tree/297483c11265050542d7ef107d14c44936c51883
---

# Micro Frontends + single-spa：用垂直业务切片约束多 Agent 所有权

微前端常被误解为“在一个页面里运行多个框架”的前端技巧。Cam Jackson 的文章和 single-spa 的契约都指向更严格的目标：把大型产品拆成可由独立团队端到端负责、可独立构建与发布、在浏览器运行时组合的业务切片。框架只是实现手段；组织交付边界才是首要问题。

本案例把这种架构当作多 Agent 系统的受限类比。一个完整业务切片不只是一个提示词或一个模型调用，而应同时拥有领域任务、专用工具、局部记忆、评估、运行指标和发布节奏；共享 Shell 只负责组合、身份上下文、顶层路由和系统级门禁。若所有切片仍读写同一个全局状态对象、依赖隐式事件顺序并锁步发布，进程虽然分开，系统仍是分布式单体。

## 学习问题

1. 为什么微前端首先解决的是多团队并行交付，而不是框架混搭或代码体积问题？
2. 垂直业务切片如何同时约束 UI、领域逻辑、数据访问、运行责任和发布权限？
3. single-spa 的 root config、`activeWhen` 与 `bootstrap`、`mount`、`unmount`、`unload` 分别拥有哪部分控制权？
4. 跨应用通信、共享依赖、认证与路由怎样保持显式契约，而不重新形成锁步发布？
5. 生命周期拒绝、超时、加载失败和清理失败会进入哪些状态，平台还必须补齐什么恢复能力？
6. 将一个完整业务切片映射为 Agent、工具与记忆的所有权时，哪些类比成立，哪些会因非确定性和服务端状态而失真？
7. 什么时候一个小团队、一个代码库和一个发布节奏更简单可靠，不应为了架构时髦而拆分？

## 一页摘要

**已证实事实**：Cam Jackson 将微前端定义为把可独立交付的前端应用组合成整体，并把主要收益放在自治团队、独立部署和增量演进上。团队应围绕垂直业务能力组成，从构想到生产持续拥有一块用户价值，而不是按样式、表单或验证等水平技术能力分工。每个微前端都应有自己的持续交付流水线；运行时组合优于把所有包重新编译进一个锁步发布物。

**已证实事实**：single-spa 的 root config 由共享 HTML 与调用 `registerApplication` 的 JavaScript 组成。注册项提供名称、加载函数或应用对象、激活条件和可选 `customProps`。`activeWhen` 根据当前 `location` 判断应用是否应活跃；`start()` 之前应用可以加载，但不会 bootstrap、mount 或 unmount。应用必须提供异步 `mount` 与 `unmount`，文档也把 `bootstrap` 列为注册应用的标准生命周期，`unload` 可选；源码会把缺失的 `bootstrap` 或 `unload` 展平为成功的空操作。
...
**个人分析**：映射到多 Agent 系统时，应把“一个可独立交付的前端业务域”类比为“一个拥有完整业务结果的 Agent 切片”：例如退款切片拥有退款判断 Agent、支付/工单工具适配器、退款任务状态与局部长期记忆、领域评估集、SLO 和发布权限。共享编排 Shell 只做身份、顶层意图路由、预算与策略门禁、结果装配。把所有 Agent 都连接到一个无版本全局记忆、一个万能工具网关和一个同步发布列车，会让任一字段变化牵动全局，形成分布式单体。
...
**已证实事实**：single-spa 把浏览器中的微前端分为 application、parcel 与 utility module。application 按一组路由声明式激活并由 single-spa 管理生命周期；parcel 以命令式方式嵌入、不拥有路由，主要用于跨框架 UI；utility module 暴露公共 JavaScript 接口，不直接进入 single-spa 生命周期。官方同时提醒，浏览器标签页中的这些模块共享同一进程、主线程和 DOM，并不具备后端微服务的进程或数据库隔离。
...
**基于证据的推断**：浏览器内运行时组合隔离的是代码库、流水线和责任，不自动隔离 CPU、内存、DOM、Cookie、Web Storage、网络会话或供应链权限。一个失控应用仍可阻塞主线程、覆盖全局样式、读取同源令牌或发出高成本请求。因此，“独立部署”不能被误写成“安全沙箱”；需要 iframe、不同 origin、内容安全策略或后端授权边界时，应明确采用额外隔离手段。
...
```mermaid
flowchart TB
  accTitle: 微前端运行时组合与 Agent 垂直所有权映射
  accDescr: 共享 Shell 根据路由和身份激活各业务切片；每个切片拥有自己的界面、领域逻辑、数据接口和发布流水线，并映射为拥有工具和局部记忆的 Agent 业务切片。

  User["用户"] --> Shell["root config / 共享 Shell"]
  Shell --> Identity["会话身份与顶层策略"]
  Shell --> Router["URL / activeWhen"]
  ...
  Orders --> OrdersUI["UI + 内部路由"]
  Orders --> OrdersAPI["订单 API / BFF"]
  Refunds --> RefundUI["UI + 内部路由"]
  Refunds --> RefundAPI["退款 API / BFF"]
  Profile --> ProfileUI["UI + 内部路由"]
  Profile --> ProfileAPI["账户 API / BFF"]

  Shared["版本化 utility modules\n认证 helper / 设计系统 / 错误接口"]
  Shared -.显式导入.-> Orders
  Shared -.显式导入.-> Refunds
  Shared -.显式导入.-> Profile

  Orders -.垂直映射.-> OrderAgent["订单 Agent 切片\n任务 + 工具 + 局部记忆 + 评估"]
  Refunds -.垂直映射.-> RefundAgent["退款 Agent 切片\n任务 + 工具 + 局部记忆 + 评估"]
  Profile -.垂直映射.-> ProfileAgent["账户 Agent 切片\n任务 + 工具 + 局部记忆 + 评估"]

  PipelineA["独立 CI/CD"] --> Orders
  PipelineB["独立 CI/CD"] --> Refunds
  PipelineC["独立 CI/CD"] --> Profile
...
一次 URL 导航不是容器直接命令某个应用渲染。**已证实事实**：注册后，single-spa 依据状态与 `shouldBeActive` 把应用分类为待加载、待挂载、待卸载或待 unmount；路由器允许新应用 load/bootstrap 与旧应用 unmount 并行，但会等待全部旧应用完成 unmount，才真正 mount 新应用。加载期间用户若再次改变路由，源码会在 bootstrap 前和 mount 前重复检查激活条件，避免迟到的应用挂载到错误页面。

```mermaid
sequenceDiagram
  accTitle: single-spa 路由激活、生命周期与失败状态
  accDescr: root config 注册应用并启动运行时；导航触发激活判断，旧应用先清理，新应用加载和初始化，只有仍满足激活条件时才挂载；生命周期失败进入显式错误状态。

  actor U as 用户
  participant RC as root config
  participant SS as single-spa
  participant Old as 旧业务切片
  participant New as 新业务切片
  participant Obs as 错误与观测

  RC->>SS: registerApplication(name, load, activeWhen, props)
  RC->>SS: start()
  U->>SS: pushState / popstate / hashchange
  SS->>SS: getAppChanges + shouldBeActive
  par 清理旧应用
    SS->>Old: unmount(props)
    alt Promise 成功
      Old-->>SS: NOT_MOUNTED
    else 拒绝或致命超时
  ...
  and 准备新应用
    ...
    alt 网络或动态导入失败
      ...
      SS->>SS: 再次检查 activeWhen
      SS->>New: bootstrap(props)
      New-->>SS: NOT_MOUNTED
    end
  end
  SS->>SS: 等待旧应用全部 unmount
  SS->>SS: mount 前再次检查 activeWhen
  alt 仍需激活
    SS->>New: mount(props)
    alt Promise 成功
      New-->>SS: MOUNTED
...
本案例钉扎 single-spa 仓库提交 [`297483c11265050542d7ef107d14c44936c51883`](https://github.com/single-spa/single-spa/tree/297483c11265050542d7ef107d14c44936c51883)，仓库 `package.json` 的版本为 6.0.3。该提交本身只更新资助配置，但它固定了本次分析使用的完整源码快照；以下结论都以该 SHA 为准。
...
**基于证据的推断**：single-spa 的核心不是一个任意插件加载器，而是带错误状态的路由驱动状态机。状态检查使重复调用趋于无操作，reroute 的等待队列避免多个导航同时改写生命周期；但它没有事务性回滚整个页面，也不把一个应用的失败自动替换为备用应用。生产 Shell 必须定义 error boundary、占位 UI、重试、回滚与用户可见降级。

**个人分析**：对 Agent 平台，最值得移植的是“注册合同 + 激活谓词 + 明确状态 + 清理屏障 + 错误分类”。不应移植默认的轻量超时语义：工具调用可能产生计费、发信、付款或数据修改，超时后必须通过幂等键、查询状态或补偿确认结果，而不能仅记录 console error 并继续等待。
...
**个人分析**：对 Agent 系统，按“研究 Agent、写作 Agent、工具 Agent”水平拆分，类似按样式、表单、验证拆前端团队：一次业务结果必须跨多个所有者排队。更合理的垂直切片是“退款解决”“合同审查”“客户续约”等，每个切片内部可以包含研究、推理和行动能力，却由一个团队承担结果、工具权限、记忆质量和事故响应。
...
**可靠性与性能。个人分析**：应分别为网络加载、bootstrap、mount、unmount 设预算，测量每个制品的传输与执行成本，并建立长任务、内存增长、未释放监听器和主线程阻塞告警。失败应用要有可访问的占位 UI，不能让 root config 崩溃。共享依赖去重应以真实字节和兼容成本决策；过度外置会把一次局部升级变成全局变更。
