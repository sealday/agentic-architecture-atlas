...
sequenceDiagram
  ...
  accDescr: Producer 按业务 key 把任务追加到分区。Classic group coordinator 组织成员和 generation，被选出的客户端 leader 计算 assignment。Consumer poll 后登记 inbox，并在受保护目的地的 work ledger 原子取得独立 ownership epoch；目的地把 epoch 比较与业务变更放在同一原子操作中。若动作成功后、offset 提交前崩溃，rebalance 后的新 owner 会取得更高 epoch，旧 owner 的条件变更因此不产生副作用。
  ...
  participant X as Protected Destination + Work Ledger
  ...
  C1->>I: INSERT inbox(E)，唯一约束确认首次登记
  I-->>C1: inbox E 已持久化
  C1->>X: 原子取得 E 的 work epoch F
  X-->>C1: 当前 epoch F
  C1->>X: conditional_mutate(E, expected_epoch=F)
  X->>X: 同一原子操作：比较 F 并应用业务变更
  X-->>C1: epoch 匹配，业务变更已提交
  ...
  C2->>I: INSERT inbox(E)
  I-->>C2: E 已存在；读取已有状态
  C2->>X: 原子接管 E，work epoch F → F+1
  X-->>C2: 当前 epoch F+1
  C1-->>X: conditional_mutate(E, expected_epoch=F)
  X->>X: 同一原子操作：比较到 F+1，不应用业务变更
  X--xC1: 旧 F 条件变更被拒绝
  C2->>G: OffsetCommit(next=42, generation N+1)
```

文字等价描述：producer 只负责把带稳定 `event_id` 与 partition key 的事实追加到日志；classic group 的 leader client 计算 assignment，broker coordinator 管理 Kafka generation 并传播结果。consumer 先把事件登记到 durable inbox，再从受保护目的地的应用 work ledger 原子取得 epoch `F`。目的地必须在同一原子操作中比较“当前 epoch 仍为 `F`”并应用业务变更，不能先校验再另行写入。若旧 owner 在业务变更成功后死亡，新 owner 会从旧 committed offset 重读，并原子接管为 `F+1`；此后旧 owner 携带 `F` 的条件变更在目的地内比较失败，业务变更不会发生。Kafka generation `N/N+1` 只出现在 group 协议与 OffsetCommit 校验中，不充当外部业务 fence；最后才提交下一 offset。若目的地无法将 epoch 比较与业务变更原子绑定，则这套方案不提供真正 fencing，只能依靠幂等去重、dispatcher、补偿与对账降低重复副作用。
...
**fencing 与取消。** 受保护目的地或拥有权威业务变更的 ledger/dispatcher 为每次 owner/attempt 提升 epoch；所有状态写携带 `(task_id, epoch)`，并在同一事务或条件写中原子比较 epoch 与应用业务变更，旧 epoch 影响零行即停止。若目的地只能先查询 epoch、随后单独执行变更，两步之间的 takeover 会形成 TOCTOU，因此不是真正 fencing。对无法原子校验的邮件、支付或外部工具，只能使用幂等键、单独 side-effect dispatcher、审批、补偿和 reconciliation 缓解重复风险。rebalance callback 可以停止接收、flush 安全进度和释放本地资源，但无法强杀已发出的 HTTP 请求；取消必须是协作式，并由最终状态对账。
