...
先看稳定状态下的 ownership：一个 group 内，partition 如何分给 consumer？图中每条 assignment 线都只指向一个当前读取者。

![同一消费组内分区读取所有权的划分](/img/illustrations/apache-kafka-consumer-groups-partition-ownership.png)

这张图只说明当前 assignment 的取数所有权。它不把 assignment 解释成业务锁，也不证明外部副作用只发生一次。

再看 membership 变化后的状态转换：稳定消费如何经过撤销与重新分配，回到新的稳定状态？红色虚线单独标出旧 owner 的在途动作，因为 rebalance 不会自动撤回已经发出的业务请求。

![成员变化触发撤销和重新分配的 rebalance 循环](/img/illustrations/apache-kafka-consumer-groups-rebalance-loop.png)

这个循环不是无缝切换承诺。新 owner 恢复消费与旧工作仍可能完成可以同时成立，业务目的地仍需幂等或 fencing。
