...
official_sources:
  ...
  - https://github.com/temporalio/sdk-go/tree/3cebc26908119368df3911df6146f756bad3882b
  - https://github.com/temporalio/sdk-go/blob/3cebc26908119368df3911df6146f756bad3882b/internal/internal_worker_heartbeat.go
  - https://github.com/temporalio/sdk-go/blob/3cebc26908119368df3911df6146f756bad3882b/internal/internal_task_handlers.go
  - https://github.com/temporalio/sdk-java/tree/6d09a341c14daeddf9b3795cd43721de770fec2c
  - https://github.com/temporalio/sdk-java/blob/6d09a341c14daeddf9b3795cd43721de770fec2c/temporal-sdk/src/main/java/io/temporal/worker/WorkerFactory.java
  - https://github.com/temporalio/sdk-java/blob/6d09a341c14daeddf9b3795cd43721de770fec2c/temporal-sdk/src/main/java/io/temporal/internal/worker/WorkerCommandTaskHandler.java
  - https://github.com/temporalio/sdk-java/blob/6d09a341c14daeddf9b3795cd43721de770fec2c/temporal-sdk/src/main/java/io/temporal/internal/activity/ActivityExecutionContextFactoryImpl.java
  - https://github.com/temporalio/sdk-java/blob/6d09a341c14daeddf9b3795cd43721de770fec2c/temporal-sdk/src/main/java/io/temporal/internal/activity/HeartbeatContextImpl.java
---
...
本案例以 Garcia-Molina 与 Salem 的 1987 年《Sagas》、Temporal 官方概念文档，以及 `temporalio/temporal@955948007cc6d9d94fa8ef484225954bd9328451` 为服务端证据截面；取消交付的能力边界另以 `temporalio/sdk-go@3cebc26908119368df3911df6146f756bad3882b` 和 `temporalio/sdk-java@6d09a341c14daeddf9b3795cd43721de770fec2c` 核对。Workflow 语言级重放由 SDK 完成，因此本文用官方 Workflow/Event History 文档证实重放契约，用固定服务端源码定位 Command、Activity 持久状态、重试、计时任务与 Task Queue 接缝，只在取消小节引用上述两个 SDK 的具体实现，不把服务端误说成 SDK 解释器，也不把两个 SDK 当前行为外推到所有 SDK/版本。
...
- Activity cancellation 是协作式请求。官方 Activity 文档把 Activity heartbeat 描述为通用的取消交付基线/回退路径；但在本文固定的服务端与 Go/Java SDK 提交中，若 namespace/server 报告 Worker Commands 能力且 Worker 注册了 `WorkerControlTaskQueue`，服务端可发送带 task token 的 `CancelActivityCommand`，兼容 SDK 将它映射到正在运行 Activity 的取消 callback，不必等待下一次 Activity heartbeat。这个快速路径依赖服务端能力、SDK 实现与部署版本，不能外推到所有 SDK/版本；无论走哪条路径，Activity 仍可选择如何响应，取消都不是强制 kill 或外部副作用回滚。
...
flowchart LR
    ...
    subgraph EFFECT["非确定副作用边界"]
        ...
        G -->|"是：读取原结果"| LEDGER
        G -->|"否"| EXT --> LEDGER
        LEDGER -->|"outcome / effect_ref"| AW
        LEDGER -. "外部成功但 Worker 尚未回执" .-> WINDOW
        AW -. "在 completion RPC 前崩溃" .-> WINDOW
    end

    TS --> ATQ
    AW -->|"完成 / 失败 / heartbeat RPC"| TS
    TS -->|"超时或可重试失败：新 attempt"| ATQ

    subgraph SAGA["Saga 向后恢复"]
        F{"Ti 有可对账成功证据？"}
        PUSH["记录 Ti / Ci / effect_ref"]
        POP["Workflow 逆序选择 Cj ... C1"]
        CCMD["Command：调度补偿 Activity\n同样幂等、可重试"]
        HUMAN["冻结新副作用\n对账 + 人工修复"]
        F -->|"是"| PUSH
        F -->|"未知"| HUMAN
        POP --> CCMD
    ...
    ST -->|"后续步骤失败"| POP
    CCMD --> TS
    ST -->|"补偿 confirmed：弹栈并选下一项"| POP
    ST -->|"补偿未知/永久失败/不可补偿"| HUMAN
    HUMAN -->|"人工决策作为新事件"| C
```
...
| Heartbeat | 报进度，可带下次 attempt 恢复用 payload，也是官方文档中的取消交付基线/回退点；兼容 Worker Commands 的部署可另行触发 callback | 只记录可安全恢复的进度和外部引用，并验证当前服务端/SDK 是否启用快速取消路径 | 把 heartbeat 当作唯一取消通道或跨系统事务提交 |
...
**取消与补偿分流。** Workflow 取消可以停止尚未调度的正向步骤，并请求运行中 Activity 协作停止。交付可能来自 heartbeat 基线/回退，也可能在本文固定、能力兼容的 Go/Java SDK 路径中来自 Worker Command callback；后者减少等待下一次 Activity heartbeat 的延迟，却仍只是通知正在运行的代码。已获得 `effect_ref` 的子事务要走补偿路径；取消只表达“不要继续”，补偿才表达“用新业务动作修正已发生的事实”。补偿 Activity 也会超时、重试、重复执行或遭遇永久错误，所以也需幂等键、独立预算和人工终态。

## 关键源码导读

服务端链接固定到 `temporalio/temporal@955948007cc6d9d94fa8ef484225954bd9328451`；取消快速路径另固定到 `sdk-go@3cebc26908119368df3911df6146f756bad3882b` 与 `sdk-java@6d09a341c14daeddf9b3795cd43721de770fec2c`。服务端源码展示 Command、Activity mutable state、retry 与 matching；SDK 链接只证明这两个固定版本的 Worker Command/callback 路径，不用于外推 Workflow replay 实现或其他 SDK 行为。
...
| [`workflow_task_completed_handler.go` 670–768](https://github.com/temporalio/temporal/blob/955948007cc6d9d94fa8ef484225954bd9328451/service/history/api/respondworkflowtaskcompleted/workflow_task_completed_handler.go#L670-L768) | 取消请求先入历史；未开始 Activity 立即取消；已开始且记录了 control queue/clock 时，服务端按 queue 汇集 `CancelActivityCommand` 并生成 Worker Commands tasks | 快速交付是能力/版本相关分支；“请求取消”、“callback 触发”与“外部效果已停止”仍是三个状态 |
...
5. **Heartbeat 类比 checkpoint。** 它可保存 Activity 进度，也是官方取消交付基线/回退；本文固定的兼容 Go/Java SDK 还可通过 Worker Command callback 更快交付，但两条路径都不代表外部系统与 Temporal 原子提交或强制终止。
...
4. **不要把 timeout 或 cancellation 当成旧执行已终止。** 超时后可与新 attempt 并存；取消可经 heartbeat 基线/回退或能力兼容的 Worker Command callback 交付，但都依赖 Activity 代码协作，外部状态未知时先对账。
...
以下均为经典论文、Temporal 官方文档或上游源码。访问日期与来源截断日期：**2026-07-22**。当日远程 HEAD 分别解析并固定为：`temporalio/temporal@955948007cc6d9d94fa8ef484225954bd9328451`、`temporalio/sdk-go@3cebc26908119368df3911df6146f756bad3882b`、`temporalio/sdk-java@6d09a341c14daeddf9b3795cd43721de770fec2c`。
...
- [Temporal Activity](https://docs.temporal.io/activities)、[Activity Definition](https://docs.temporal.io/activity-definition) 与 [Activity Execution](https://docs.temporal.io/activity-execution) — Activity 非确定性、已完成结果的 replay 复用、Worker 在报告前崩溃导致重执行、幂等键、Activity ID，以及 heartbeat 作为官方文档中的协作式取消基线/回退。
- [Temporal Retry Policies](https://docs.temporal.io/encyclopedia/retry-policies) 与 [Detecting Activity Failures](https://docs.temporal.io/encyclopedia/detecting-activity-failures) — Activity/Workflow 默认重试差异、退避参数、non-retryable errors、四类 Activity timeout、heartbeat 进度和文档基线取消交付。
...
- [`temporalio/sdk-go` 固定提交](https://github.com/temporalio/sdk-go/tree/3cebc26908119368df3911df6146f756bad3882b) 与 [`temporalio/sdk-java` 固定提交](https://github.com/temporalio/sdk-java/tree/6d09a341c14daeddf9b3795cd43721de770fec2c) — 仅用于证明这两个版本在服务端/namespace 宣告 Worker Commands 能力时，可通过 control queue 的 `CancelActivityCommand` 触发运行中 Activity callback，而不必等待下一次 Activity heartbeat；不代表所有 SDK/版本都具备该路径。
