...
```mermaid
flowchart LR
    C["client request"] --> CMD["Workflow command"]
    CMD --> H["durable history"]
    H --> WTQ["Workflow Task Queue"]
    WTQ --> WW["Workflow Worker"]
    WW -->|"deterministic replay"| CMD

    CMD --> ATQ["Activity Task Queue"]
    ATQ --> A["Activity task"]
    A --> K{"stable operation key\n已有结果？"}
    K -->|"是"| L["读取 effect_ref"]
    K -->|"否"| E["external side effect"]
    E --> L
    L --> R["completion RPC"]
    E -. "worker crash\n回执前失败" .-> U["unknown"]
    ...
    Q -->|"confirmed"| L
    Q -->|"not found"| A
    Q -->|"ambiguous"| M["manual escalation"]
