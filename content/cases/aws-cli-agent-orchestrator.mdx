...
**已证实事实**：CAO 当前还提供 workflow script 层，以稳定 `run_id` / `step_id`、SQLite journal、取消和 resume 支持长流程。resume 会从头重放脚本，但对已完成且 fingerprint 一致的步骤返回 journal 结果；它不是恢复原 CLI 进程内存。普通终端删除快照只能恢复 scrollback 到一个普通 shell，进程崩溃本身不保存快照。

**基于证据的推断**：CAO 最适合作为“可信开发机上的多 CLI 编程工作台”。它擅长把研究、实现、审阅和汇总分派给不同 CLI agent，并让人随时 attach 观察；若要进入共享服务或高风险生产环境，还需要在 CAO 外部增加强身份、仓库写入租约、worktree/container 隔离、秘密代理、审计存储和业务幂等账本。
...
| 安全 | profile 工具限制、路径校验、启动确认 | 统一强制策略、细粒度 MCP 授权、tenant 隔离 |
...
- workflow script 通过独立进程调用 `cao_workflow` shim；脚本需先 validate，并用预先声明的 `run_id` 启动。并发 `run_step` 必须使用稳定显式 `step_id`，否则恢复时线程调度顺序可能把 journal 结果配错。
- workflow resume 会重新执行脚本并重放已完成步骤；脚本顶层的时间、随机数或 UUID 可能造成 replay divergence。journal 提升流程恢复性，但不会回滚已发生的 Git push、文件覆盖、消息发送或云资源修改。
...
- `CAO_TERMINAL_ID` 适合做本地路由关联键，不应直接当成跨用户认证凭证。固定版本源码注释也明确指出 `caller_id` 不是经认证/授权的字段，这符合单用户本地信任模型，而不是多租户边界。
...
4. ERROR、timeout 与 workflow restart 是不同故障。前两者要求检查活终端和外部副作用；workflow resume 只能按 journal 重放确定性 step 结果，不能自动回滚文件系统或第三方系统。
...
**已证实事实**：MCP 的标准边界是 host–client–server。CAO 为 session 内 agent 暴露 `cao-mcp-server`，提供编排 tools；另有 `cao-ops-mcp` 供外部 MCP-capable agent 管理 session。两者最终都调用 CAO 本地 HTTP API，而 worker 本身仍由各 provider CLI 启动。
...
9. [`docs/workflows.md`](https://github.com/awslabs/cli-agent-orchestrator/blob/bae80071a17e001380367c461b32d64bc6b54433/docs/workflows.md) 与 [`services/workflow_journal.py`](https://github.com/awslabs/cli-agent-orchestrator/blob/bae80071a17e001380367c461b32d64bc6b54433/src/cli_agent_orchestrator/services/workflow_journal.py)：理解 validate、run/status/cancel/resume、稳定 step ID、journal source of truth 和 deterministic replay。
...
临时一次性协作可直接用三类原语与 terminal status；需要跨进程重启、可查询 run ID、多个确定步骤或取消时，应使用 workflow journal。两者仍不替代业务事务：journal 记录 agent step，业务数据库记录外部事实。
...
**已证实事实**：CAO 默认本地 HTTP 服务和终端模型面向受信任机器；`caller_id` / terminal ID 用于路由而非强认证。working directory 验证拒绝系统敏感路径，但允许用户 home、外部卷和其他有效目录。`--yolo` 允许任意工具并跳过确认。

**个人分析**：共享机器或服务化部署至少补齐：

- API 监听限制、强身份、每个 session/terminal 的 owner 与 tenant 授权；
...
| workflow runner 中断 | resume 重放 completed steps，继续其余步骤 | 非确定脚本、外部副作用 exactly-once |
| 主机丢失 | 取决于备份 | 自动跨主机迁移与灾备 |

**个人分析**：重试前先做 reconciliation：检查目标文件 hash、Git status、外部 API 的 idempotency key、inbox 回调和 journal step。无法判定时进入人工审核，不要让另一个 worker盲目覆盖现场。
...
- supervisor 为每个派发生成稳定 `task_id`，重试沿用同一业务幂等键并增加 attempt；
...
6. **恢复依赖确定性。** durable journal 只有在 step ID、输入和控制流稳定时才能安全 replay；随机、时间和并发调度都要从业务 ID 派生或显式记录。
...
5. [Terminal Lifecycle](https://github.com/awslabs/cli-agent-orchestrator/blob/bae80071a17e001380367c461b32d64bc6b54433/docs/terminal-lifecycle.md) 与 [Workflows](https://github.com/awslabs/cli-agent-orchestrator/blob/bae80071a17e001380367c461b32d64bc6b54433/docs/workflows.md)：区分快照取证、流程 journal、cancel 与 resume。
6. [MCP 上游固定版本仓库](https://github.com/modelcontextprotocol/modelcontextprotocol/tree/46fa5192d4969f51ecea9896f7211a67d147f803) 与 [MCP 2025-06-18 架构规范](https://modelcontextprotocol.io/specification/2025-06-18/architecture)：把 CAO MCP server 放回 host–client–server 和 capability negotiation 的协议上下文。
