...
**基于证据的推断**：因此超时关闭 HTTP 连接、用户点击“停止”和远端副作用停止是三件不同的事。版本与源码入口由固定证据截面核验；沿着一次本地委派跨到远端服务时，正文持续追问信任、取消、工具副作用和人工对账由谁承担。
...
这一取消边界不会因为任务已进入 A2A 而消失。需要取消时，应用必须自建支持该操作的 A2A client/gateway，并在接入时验证远端实现。
