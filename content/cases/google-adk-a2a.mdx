...
本文只分析固定证据截面。版本与源码入口放进证据卡；远端信任、取消后果、工具副作用和人工对账继续留在主叙事。
...
**已证实事实**：本地 `BaseAgent` 以 `sub_agents` 形成单父树，`transfer_to_agent` 移动下一段 agent loop 的控制。Sequential、Parallel 与 Loop 则用代码固定顺序、扇出和循环外壳；流程确定不代表子 agent 输出确定。
...
**基于证据的推断**：本地层级与 A2A 联邦是两个治理层。根 agent 拥有本地预算和最终回答，远端服务拥有自己的任务、工具和恢复；协议只连接两侧，不合并状态所有权。
...
**说明性场景｜本地 Root 先运行确定性校验，再把一个长任务委派给远端 agent。** 该场景只组合 ADK tree、Session 和 A2A Message/Task 机制，不代表真实客户或生产事故。

本地 Root 先校验用户与预算，并决定远端 skill 是否在允许列表。`transfer_to_agent` 只改变本地当前处理者；进入 `RemoteA2aAgent` 后，调用已经跨越网络和独立身份边界。

远端可以返回 Message，也可以创建 Task 并进入 working、input-required 或 auth-required。此时本地 Session 只保存远端 ID 映射，Task 的真实状态、内部工具与恢复策略仍归远端服务。

当远端完成并返回 Artifact，本地还要校验来源、schema 和业务状态。用户取消本地等待不代表远端停止；固定 adapter 缺少端到端取消时，系统必须标记“远端可能继续”并安排查询、补偿或人工对账。
...
2. **把父子树当所有权结构。** 一个实例一个 parent、名称唯一、可达路径受限，减少隐式共享和不可解释 transfer。
3. **把确定性放在可证明处。** 顺序、审批、预算、并发、循环终止和发布由代码/workflow 控制。
