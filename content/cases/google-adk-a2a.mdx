...
flowchart TB
  ...
  Runner --> Root["Root LlmAgent：意图、委派、最终回答"]
...
sequenceDiagram
  ...
  accDescr: 本地根智能体先经可信注册表或已配置地址取得并验证 Agent Card，再获取凭证并发送消息；远端可创建任务、流式发送状态和制品，客户端可尝试取消或在需要输入时继续任务，但本地与远端分别提交状态。
...
4. `input-required` 与 `auth-required` 是可继续的中断状态；completed、failed、canceled、rejected 是终态。取消只是一项请求，调用者必须接受“无法取消或已经完成”的结果。
...
**已证实事实**：A2A Cancel Task 只要求服务尝试取消，取消可能不受支持或任务已终止；Send Message 仅 MAY 通过 `messageId` 去重。ADK 当前 executor 还没有实现 cancel。因此超时关闭 HTTP 连接、用户点击“停止”和远端副作用停止是三件不同的事。
...
3. Cancel 返回后继续轮询或订阅直到得到明确终态；若 adapter 不支持 cancel，标记为“本地停止等待，远端可能继续”，并触发对账。
4. Push notification 按至少一次投递设计，以 `taskId + event/version` 去重；乱序更新按服务端状态 timestamp 或明确序号处理。
