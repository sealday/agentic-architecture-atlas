...
official_sources:
  ...
  - https://github.com/yjs/yjs/tree/9c1994547d7bc86245a21e1a4c8319f056d05ecf
  - https://github.com/yjs/yjs/blob/9c1994547d7bc86245a21e1a4c8319f056d05ecf/INTERNALS.md
  - https://github.com/yjs/yjs/blob/9c1994547d7bc86245a21e1a4c8319f056d05ecf/THREAT_MODEL.md
---

# Yjs CRDT Collaboration：让人和多个 Agent 合并共享工作区状态

人和多个 Agent 同时修改同一工作区时，最先暴露的问题往往不是“谁先拿到锁”，而是状态被混成了一类：正文、结构化字段、当前光标、正在处理的段落、模型的临时计划、审批结果和外部副作用都塞进同一个对象。Yjs 提供了一组很有价值、但边界严格的机制：共享类型承载可合并的文档状态，二进制 update 允许乱序和重复交付，provider 负责传输或持久化，Awareness 传播不应跨会话保存的在线状态。

本案例把它迁移为“人 + 多 Agent 的协同工作区”模型。正文、评论、候选补丁和结构化产物可以进入持久 CRDT 文档；光标、当前关注区、短暂的“正在生成”提示和意图声明进入 Awareness。这个映射只说明如何让特定共享数据最终收敛，不代表 Yjs 会替系统完成鉴权、业务事务、语义冲突判断、审计或外部副作用去重。

## 学习问题

1. `Y.Text`、`Y.Array`、`Y.Map` 与 `Y.Xml*` 这样的 shared types 提供了什么，哪些普通 JSON 修改不会被同步？
2. `update` 的交换律、结合律与幂等性究竟保证什么，为什么不能推广为业务操作、模型结论或工具副作用的幂等？
3. state vector 如何表达本地已知状态，`encodeStateAsUpdate` 与 `diffUpdate` 如何只交换缺失差异？
4. network provider、persistence provider 与 Awareness 分别拥有哪种状态，离线副本恢复后怎样追平？
5. snapshot、删除集合与垃圾回收之间有什么约束，为什么 snapshot 不是审计日志或默认备份机制？
6. 人和多个 Agent 同时写正文时，如何把可持久内容与短暂 presence/intent 分开，并避免把“正在编辑”误当成排他锁？
7. 文档分区、鉴权、schema 演进、审计、恶意客户端与资源限制应由哪一层负责？

## 一页摘要

**已证实事实**：Yjs shared types 类似 Array、Map、Text 与 XML 数据结构，连接到 `Y.Doc` 后可以被观察，并通过 provider 自动同步与持久化。对 shared type 的修改发生在 transaction 中；显式 transaction 可以把多次本地修改合并为一次 observer 通知和一个 update。普通 JSON 对象不会因为被原地修改就产生 Yjs 事件，官方文档明确警告这可能导致副本不再同步。

**已证实事实**：Yjs 文档变更被编码为 `Uint8Array` update。官方 Document Updates 文档对这些 update 给出交换律、结合律和幂等性：同一组有效 update 在兼容的 Yjs 文档中可以乱序、重复应用，收到全部 update 后副本收敛。`applyUpdate` 负责应用 update；`encodeStateVector` 表达各 client 的下一预期 clock，远端可把它交给 `encodeStateAsUpdate` 只编码缺失结构，或对已合并的二进制状态使用 `diffUpdate`。

**边界。已证实事实**：上述代数性质属于 **Yjs document update 的集成**，不是任意 JSON 消息、模型回答、数据库写入、支付或发信的性质。固定源码的 `INTERNALS.md` 还说明 state vector 用于描述本地文档并计算远端缺失结构，不用于追踪业务因果。`Y.Doc.transact` 将本地修改打包为一个 update，也不等于跨数据库、队列和工具的分布式事务。

**已证实事实**：provider 不属于 CRDT 合并算法本身。network provider 在节点间转发 update，database provider 把 update 同步到存储。官方离线指南把 `y-indexeddb` 与 network provider 连接到同一个 `Y.Doc`：本地数据库先恢复已知内容，离线修改继续持久化；网络恢复后只同步缺失变化。Awareness 则是 `y-protocols` 中的可选 CRDT，通常由 network provider 实现，不保存在 Yjs 文档中，适合用户在线状态、光标等跨会话无须保留的信息。

**基于证据的推断**：在人与 Agent 的共享工作区中，应把“以后仍需读取、复制或审阅的内容”放进持久文档，把“当前正在看哪里、准备改哪里、一次生成尚未完成”放进 Awareness。离线 Agent 恢复后可以合并其持久草稿，但离线期间的 presence/intent 应过期重建；旧 Awareness 不能当作锁、授权或任务所有权证明。

**个人分析**：Yjs 适合协同正文、白板节点、注释和可独立合并的局部属性，不适合直接代表“批准付款”“任务只执行一次”或“最终结论正确”。两个 Agent 的文本插入可以无人工合并地收敛，却仍可能形成自相矛盾的段落；两个状态字段也可能在数据层得到确定结果，却违反审批顺序。生产系统必须在 CRDT 之外保留权限门禁、语义校验、审批状态机、幂等副作用账本与审计链。
...
本案例在 2026-07-22 访问 Yjs 官方文档，并以当日 `git ls-remote https://github.com/yjs/yjs.git HEAD` 返回的提交 [`9c1994547d7bc86245a21e1a4c8319f056d05ecf`](https://github.com/yjs/yjs/tree/9c1994547d7bc86245a21e1a4c8319f056d05ecf) 固定源码快照。该提交时间为 2026-07-18，仓库 `package.json` 版本为 `14.0.0-rc.24`。固定 HEAD 用于说明当前源码和内部结构，不是生产采用候选版的建议；同一快照的 `SECURITY.md` 仍只把 13.6.x 列为受支持版本，选型时应另行核对正式支持版本。

**已证实事实**：`INTERNALS.md` 把 Yjs 核心描述为 list CRDT：数组是任意项目列表，文本是字符、格式标记与嵌入的列表，Map 的同键条目以最后插入条目为可见值，其余重复项标记删除。插入内容由 `(clientID, clock)` 唯一标识，并借助 `origin` 与 `originRight` 在同一位置并发插入时得到一致顺序。删除采用不同路径：删除集合随 update 传播；Yjs 不记录删除发生时间或删除者。
...
**已证实事实**：Yjs 本身不规定网络拓扑。y-websocket 使用中心 WebSocket 端点分发文档 update 与 Awareness，文档称它适合在中心端处理认证和授权，因为 WebSocket 可携带 header 和 cookie；这只是部署接缝，不是自动鉴权。固定 `THREAT_MODEL.md` 明确把 peer 认证授权、TLS/WSS、服务端访问控制和应用输入校验列为应用责任，并警告任何拥有写权限的恶意 peer 都可能破坏原文档。
...
一个可恢复的多人/多 Agent 编辑回合可以按以下顺序运行：
...
固定源码快照为 [`9c1994547d7bc86245a21e1a4c8319f056d05ecf`](https://github.com/yjs/yjs/tree/9c1994547d7bc86245a21e1a4c8319f056d05ecf)。以下只描述该快照以及 2026-07-22 访问到的官方文档；HEAD 之后的实现和文档变化不在结论范围内。
