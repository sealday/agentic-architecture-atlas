...
**已证实事实**：一个 Durable Object class 对应一个 namespace，每个对象有自己的 ID、活动实例和私有持久存储。`idFromName()` 从稳定名称得到 ID，`newUniqueId()` 生成随机 ID，`get()` 返回 stub；真正调用时才路由并懒创建。stub 是远程 client，不是对象本身，失败后应重新获取。
...
**已证实事实**：SQLite-backed 对象提供 SQL、同步/异步 KV、事务与 PITR。Input Gate 控制 storage await 期间的输入交错，Output Gate 延迟写入后的外发消息，直到 durable write 确认。Output Gate 不会撤销写入前已发出的 HTTP 请求，也不会把 LLM、R2、队列、付款或邮件纳入对象事务。
...
固定快照：[`cloudflare/workerd@14894eccbb339da199d772fcadd337b6d0902128`](https://github.com/cloudflare/workerd/tree/14894eccbb339da199d772fcadd337b6d0902128)，Release 2026-07-22。
