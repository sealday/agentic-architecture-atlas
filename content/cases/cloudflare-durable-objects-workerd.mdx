...
official_sources:
  ...
  - https://developers.cloudflare.com/changelog/product/durable-objects/
  ...
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/src/workerd/io/io-context.c%2B%2B
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/src/workerd/io/actor-cache.h
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/src/workerd/io/actor-sqlite.c%2B%2B
...
**已证实事实**：当前新 `exports` namespace 只能选择 SQLite backend。2026-07-09 官方 changelog 对 legacy `migrations` 给出更窄的兼容例外：账户只要已有至少一个 KV-backed namespace，目前仍可用 `new_classes` 新建 KV namespace；没有既有 KV-backed namespace 的账户不能再新建，已有 KV namespace 继续受支持。SQLite-backed 对象同时有 SQL、同步 KV、异步 KV 和 PITR。`transactionSync()` 仅 SQLite 可用且回调必须同步；`transaction()` 仍可用，但 SQLite 下 `txn` 参数已经 obsolete，直接在 `ctx.storage` 上执行的 storage 操作属于该事务。连续、无中间 `await` 的写会自动合并并原子提交。
...
**安全。已证实事实**：每个对象的 attached storage 是私有的，其他对象不能直接访问。官方 Data security 说明全部 Durable Object 数据及其 metadata 都会自动静态加密，Worker / Cloudflare network node 与 Durable Object 之间的传输由 TLS/SSL 保护。官方 best practices 把 Worker 放在认证、校验和响应格式层；这些平台能力不等于应用授权。
