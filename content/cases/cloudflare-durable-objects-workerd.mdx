...
official_sources:
  ...
  - https://github.com/cloudflare/workerd/tree/14894eccbb339da199d772fcadd337b6d0902128
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/README.md
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/src/workerd/io/io-gate.h
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/src/workerd/io/actor-cache.h
  - https://github.com/cloudflare/workerd/blob/14894eccbb339da199d772fcadd337b6d0902128/src/workerd/server/workerd.capnp
---

# Cloudflare Durable Objects + workerd：把边缘协调收敛到身份寻址的状态单元

多客户端或多个 Agent 需要围绕同一会话、任务、租户或业务实体协调时，最难的往往不是“在哪里运行代码”，而是怎样把同一份状态稳定地送到同一个协调者。Cloudflare Durable Objects（DO）把一个全局可寻址身份、一个事件循环、私有持久存储、定时唤醒和长连接入口组合成状态单元；大量对象再以业务 key 横向分片。

这个模型的价值来自边界清晰：同一个对象适合收敛必须串行判定的状态，不同对象可以并行扩展。边界也同样严格：单线程不等于没有异步竞态，存储事务不覆盖外部模型或工具，位置 hint 不构成放置承诺，一个热点对象也不会自动拆分。开源 `workerd` 能说明 Workers runtime、I/O gates 与本地 Durable Object 存储的公开实现接缝，却没有公开或复刻 Cloudflare 生产环境的全球放置、对象迁移、复制和存储控制面。

## 学习问题

1. `idFromName()`、`newUniqueId()`、`get()` 与 stub 分别负责身份、索引保存和调用路由中的哪一步？
2. “每对象单线程”在 JavaScript 事件循环里保证什么，为什么外部 `fetch()` 等非存储 `await` 仍会引入交错与竞态？
3. Input Gate、Output Gate、SQLite/KV API、`transaction()` 与 `transactionSync()` 的原子性和可见性边界在哪里？
4. alarm 的 at-least-once 与有限重试、WebSocket Hibernation 的内存丢失、对象重启分别要求怎样的幂等和恢复协议？
5. object-per-session、task、tenant、entity 四种 key 如何影响热点、授权半径、状态迁移和重分片成本？
6. `locationHint`、jurisdiction、对象 ID、class/namespace lifecycle 和代码版本之间哪些是强约束，哪些只是提示或部署协议？
7. 怎样把 Durable Objects 映射为 Agent 会话/任务协调者，同时不把它误写成工作流引擎、全局事务或自动多租户安全边界？

## 一页摘要

**已证实事实**：一个 Durable Object class 对应一个 namespace，namespace 内的每个对象有自己的 ID、活动实例和私有持久存储。`idFromName(name)` 从稳定名称得到对象 ID，`newUniqueId()` 生成随机 ID并要求调用方把字符串索引另存；`get(id)` 立即返回 stub，真正的方法调用才触发路由与懒创建。`getByName(name)` 是当前更短的等价入口。stub 是远程调用 client，不是对象本身；同一 stub 的调用具有 E-order 交付顺序，而 stub 一旦抛出异常，后续调用也会失败，调用方要重新取得 stub。

**已证实事实**：单个对象固有单线程，但多个事件可以在异步边界交错。异步 storage 操作默认启用 Input Gate，storage 完成前暂停其他 I/O 事件；同步 SQLite 操作不让出事件循环。非存储 I/O，例如 `fetch()` 或写 R2，会允许其他请求运行，因而“读状态 → 调外部服务 → 写状态”仍可能竞态。`blockConcurrencyWhile()` 可以扩大关键区，但回调抛错或超过 30 秒会重置对象，不适合包住长时间模型推理。

**已证实事实**：新 namespace 当前应使用、且通过新 `exports` 流程只能使用 SQLite backend；legacy KV backend 仅为既有 namespace 保留。SQLite-backed 对象同时有 SQL、同步 KV、异步 KV 和 PITR。`transactionSync()` 仅 SQLite 可用且回调必须同步；`transaction()` 仍可用，但 SQLite 下 `txn` 参数已经 obsolete，直接在 `ctx.storage` 上执行的 storage 操作属于该事务。连续、无中间 `await` 的写会自动合并并原子提交。

**边界。已证实事实**：Output Gate 只把写入后的外发网络消息延迟到 durable write 确认；写失败时外发消息被丢弃、对象重置、调用方收到错误。它不撤销已经在写入前发出的 HTTP 请求，不把 LLM、R2、队列、支付或邮件纳入对象存储事务，也不能给这些系统提供 exactly-once。

**已证实事实**：每个对象同一时间只能有一个已安排 alarm。handler 是 at-least-once，未捕获异常触发从 2 秒开始的指数退避，最多 6 次 retry；因此 handler 必须幂等，有限重试耗尽后也不能假设任务完成。Hibernation WebSocket API 可以让服务端对象离开内存而连接留在 Cloudflare 网络；唤醒会重新执行 constructor，普通内存状态消失。

**基于证据的推断**：Agent 平台可把 session/task/entity ID 规范化后映射为对象名，让一个对象持有 run 状态、消息 barrier、审批版本、WebSocket 连接和 alarm。模型与工具调用仍是外部非存储 I/O，必须用 operation ID、状态版本、outbox/receipt、重试预算和补偿完成端到端恢复。
...
本案例于 **2026-07-22** 访问 Cloudflare Durable Objects 官方文档和官方 control/data plane reference architecture。同日 `git ls-remote https://github.com/cloudflare/workerd.git HEAD` 返回 [`14894eccbb339da199d772fcadd337b6d0902128`](https://github.com/cloudflare/workerd/tree/14894eccbb339da199d772fcadd337b6d0902128)；该提交题为 `Release 2026-07-22`，提交时间为 2026-07-22 00:59:49 UTC。`workerd` README 说明其版本号是日期，并对应该版本支持的最大 compatibility date；本文把固定提交视为 **2026-07-22 runtime snapshot**，不外推其后的 API 或 Cloudflare 平台行为。

**已证实事实**：`workerd` 是基于 Cloudflare Workers 同源代码的开源 JavaScript/Wasm server runtime，可用于 self-host、应用服务器和本地 Wrangler 开发。固定 README 同时警告它自身不是 hardened sandbox；固定 `workerd.capnp` 更明确写出 Durable Objects 当前始终位于单个 runtime instance，本地存储可用进程内或实验性 local-disk SQLite，并留有“未来支持跨 cluster 分布”的 TODO。

**边界。已证实事实**：这些源码能支持 I/O gate、actor cache、SQLite、本地 ID namespace 和事件循环方面的陈述，不能证明 Cloudflare 托管服务如何执行全球首次放置、故障迁移、复制、法域约束、负载调度或 storage control plane。本文不会从 `workerd` 的 local-disk 文件布局反推生产存储实现，也不会声称 self-host `workerd` 可获得托管 Durable Objects 的全球唯一性与位置语义。

**已证实事实**：官方 Data location 当前称对象创建后不会改变 location；默认靠近首次 `get()` 的请求，`locationHint` 只在该对象第一次 `get()` 时考虑，而且是 best effort、不是保证。jurisdiction 则是运行和持久化区域的约束，但 Worker 仍可从世界其他位置访问对象，且 ID 会因计费/调试在 jurisdiction 外记录。location、host 与内存生命周期不可混为一谈：对象仍会因更新、空闲或 runtime 决策被重启或换 host。
...
文字等价描述：客户端不能直接把任意 tenant/resource 字符串当作对象权限。入口 Worker 先认证主体并生成 canonical key，再通过 namespace 得到 ID 和 stub；stub 把调用路由到唯一协调单元。对象内的 storage 操作受 Input/Output Gate 约束，但外部模型和工具位于事务外。应用控制面只保存索引、shard epoch 与迁移状态，不进入每个数据请求的同步路径；热点拆分需要它显式迁移 key，平台不会自动把一个对象切开。
...
2. **选择 ID 策略。** 稳定且可重算的业务实体使用 `idFromName()` 或 `getByName()`；不可预测的新会话可用 `newUniqueId()`，但必须原子保存 ID string 到受控目录或返回给已认证客户端。ID 只在原 namespace 有效，`idFromString()` 会拒绝其他 namespace 生成的 ID。
...
固定快照：[`cloudflare/workerd@14894eccbb339da199d772fcadd337b6d0902128`](https://github.com/cloudflare/workerd/tree/14894eccbb339da199d772fcadd337b6d0902128)，Release 2026-07-22。下表只解释公开 runtime 接缝，不把本地实现描述成 Cloudflare 托管控制面。
...
**已证实事实**：固定 `workerd.capnp` 说明 Durable Object ID 与稳定 namespace `uniqueKey` 关联，class name 变化不必破坏既有 storage；Cloudflare 当前托管配置则用 `exports` 对 provisioned namespace 做 create/delete/rename/transfer reconciliation。两个证据都说明 class lifecycle 与单个对象 ID 不是同一概念，但本地 `uniqueKey` 不是用户在 Cloudflare 控制面手工操作的迁移 API。
...
**已证实事实**：当前 class lifecycle 以 `exports` 为主，legacy `migrations` 仍受支持但二者不可混用。rename 把 provisioned namespace 和数据移动到新 class name；delete 永久删除 namespace 与数据；storage backend 创建后不可原地更换；lifecycle change 只能 `wrangler deploy`，不能 gradual rollout，rollback 也不能跨越该 change。普通代码更新仍可能让新 Worker 暂时调用旧 DO 版本，API 要前后兼容。

**个人分析**：把 schema/data migration 与 class lifecycle 分开。先发布兼容读写和可重复 schema migration，记录 per-object schema version 与 migration receipt；再执行 class rename/transfer；最后移除旧接口。回滚计划必须在 lifecycle change 前准备数据 checkpoint 和前向修复，因为平台禁止简单退回跨越该边界的旧版本。
...
**安全。已证实事实**：每对象 storage 对其他对象不可直接访问，平台数据静态与传输加密；这不等于应用授权。官方 best practices 把 Worker 放在认证、校验和响应格式层。生产系统还要限制 key 长度与基数、RPC payload、WebSocket 消息、SQL 查询、外部连接和每主体成本，并让高风险 tool credential 只在实际调用时按 tenant/action 发放。
...
- [Durable Object class exports](https://developers.cloudflare.com/durable-objects/reference/durable-objects-migrations/)：2026 年当前 `exports` 与 legacy migrations、create/delete/rename/transfer、storage backend immutability、gradual deployment 和 rollback 限制。
...
**证据边界说明**：Agent 会话/任务状态机、canonical key、outbox/receipt、shard epoch、tenant directory、迁移 tombstone、资源预算和人工升级是依据官方原语形成的“基于证据的推断”或“个人分析”，不是 Cloudflare 对 Agent 平台的官方承诺。Cloudflare 托管服务未公开的全球 placement/migration/storage control plane，以及 2026-07-22 之后的文档和 `workerd` 变化，均不在本案例结论范围内。
