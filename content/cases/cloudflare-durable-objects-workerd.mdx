...
**已证实事实**：SQLite-backed 对象提供 SQL、同步/异步 KV、事务与 PITR。Input Gate 控制 storage await 期间的输入交错，Output Gate 延迟写入后的外发消息，直到 durable write 确认。若写入失败，排队的外发消息会被丢弃、对象重置、调用方收到错误；这能阻止“未持久化却已宣告成功”，但不会撤销写入前已发出的 HTTP 请求，也不会把 LLM、R2、队列、付款或邮件纳入对象事务。
