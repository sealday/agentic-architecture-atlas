...
**已证实事实**：官方 Python SDK 编排指南并列两类控制方式：由 LLM 规划、选择工具或 handoff，以及由代码确定流程。LLM 编排中最常见的两种模式是 Agents-as-Tools 与 Handoff。前者让 manager 把 specialist 当作工具调用，manager 保持对会话的控制并合成最终答案；后者让分流智能体把控制权交给 specialist，specialist 成为该次运行后续阶段的活动智能体并直接产出最终答案。官方明确允许混合这些模式。
...
| 模式 | 谁选择下一步 | 谁拥有用户会话 | 谁产出最终答案 | specialist 是否对用户“可见” | 主要优势 | 主要代价 |
| --- | --- | --- | --- | --- | --- | --- |
| Manager / Agents-as-Tools | manager LLM | manager | manager | 通常隐藏为内部工具执行者 | 可聚合多个专家、统一口径和 guardrail | manager 多一次合成调用，嵌套运行增加成本 |
| Handoff | 当前 agent 的 LLM 选择目的地 | 转移给 receiving agent | 最后活动的 specialist | 成为活动 agent，直接接管 | 专家提示更聚焦，减少 manager 转述 | 路由错误更直接，历史与 guardrail 边界更复杂 |
| code-driven orchestration | 应用代码；局部可用 LLM 分类 | 应用定义 | 应用指定的最后一步或聚合器 | 由产品界面决定 | 顺序、并行度、停止条件和预算可预测 | 工作流较僵硬，分支需要显式编码 |

一句话决策：**需要一个统一主笔就用 Agents-as-Tools；需要专家直接接管就用 Handoff；需要可预测的路径和预算就让代码编排；没有明确的专业边界就先用单智能体加工具。**
...
- Handoff 默认让 receiving agent 看到先前完整会话历史；`input_filter`、`nest_handoff_history` 和 `handoff_history_mapper` 可以改变转交内容。官方文档把嵌套 handoff 历史标为 opt-in beta，并默认关闭。
...
- agent 级 input guardrail 只作用于链上的第一个 agent，output guardrail 只作用于最终产出 agent；需要覆盖每次自定义函数工具调用时，应使用 tool guardrail。需要审批的工具调用可暂停运行，以 `RunState` 持久化、批准或拒绝后再恢复；这一中断面覆盖 handoff 后和嵌套 Agents-as-Tools 内的工具调用。

### 基于证据的推断

- “manager 拥有会话”不等于 specialist 没有运行状态。嵌套 agent 可以有自己的 session、turn limit 和 trace；区别是其产出回到 manager 的工具结果通道，用户看到的最终叙述仍由 manager 决定。
...
**个人分析**：代码外壳尤其适合分类集合稳定、步骤依赖明确、合规顺序不可变的工作流。模型可以负责“理解输入”和“生成内容”，代码负责“哪些步骤允许发生、最多发生几次、可以并发多少、失败后走哪条路径”。这样不仅降低平均成本，更重要的是收窄 P95/P99 时延与成本方差，使容量规划和性能回归更可信。
...
- provider 的 parallel tool calls 与 SDK 本地函数工具并发上限是不同控制面；前者影响模型一次能否发出多个调用，后者限制本地实际执行并发。

### Guardrail 与审批边界

agent 输入/输出 guardrail 只覆盖链首与链尾，不能假设每个 handoff agent 都自动执行两端检查。函数工具应使用 tool guardrail 做参数与结果校验；不可逆、付费、外发或高权限工具应在最接近副作用的位置要求审批。`Agent.as_tool()` 本身也可要求审批，内部工具还可再次中断；这两层分别控制“是否委派”和“是否行动”。
...
| 并行扇出过大 | 限流、下游拥塞、费用尖峰 | 固定 fan-out、SDK 本地工具并发上限、隔离舱与取消传播 |
