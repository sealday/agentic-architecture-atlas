...
这份材料不是 SDK，也不是可直接部署的系统。它更像一张企业架构问题清单：Orchestrator、Registry、记忆、通信、评估和治理各自承担什么责任，实施团队仍要自行选择技术并定义运行合同。

本文只解释固定证据截面支持的职责和边界。详细提交、文档入口与辅助链接放进证据卡；权限、工具副作用、版本兼容、故障恢复与人工升级继续留在主叙事中。
...
专业智能体准备调用写工具时，工具网关必须基于最终用户身份和当前参数再次鉴权。策略拒绝不会被模型解释为“换一种说法再试”；工具超时也不等于副作用未发生，需要用幂等键、结果查询或人工对账判定真实状态。
