...
边缘自治不是把云端控制面复制到每个现场，也不是让边缘节点在断网后获得无限权限。KubeEdge 把 Kubernetes 期望状态和设备配置留在云端规划，把已下发工作负载、局部 metadata、设备交互和运行时健康留在边缘执行；CloudHub 与 EdgeHub 恢复连接后，再通过资源版本、确认消息、状态上报和同步控制器继续收敛。

这条边界适合物理 Agent：云端决定任务、策略和版本，边缘只能在预先签发的权限租约内继续低风险动作。本文先还原 KubeEdge v1.20.0 的事实，再把离线权限、审计 outbox、冲突处置和反回滚设计明确标为跨域推断，而不是 KubeEdge 的内建能力。
...
**已证实事实**：KubeEdge v1.20.0 的 CloudCore 侧由 Kubernetes API 与 controllers 形成规划入口，CloudHub 承担云边消息中介；EdgeHub 建立 WebSocket 或 QUIC 连接并把云端资源更新路由到 EdgeCore 模块，也把节点、pod 和设备状态上报云端。连接读写或 keepalive 失败后，EdgeHub 关闭旧 client、发布 disconnected 状态、等待两个 heartbeat 周期并重连。

**已证实事实**：MetaManager 位于 EdgeHub 与 Edged 之间，把 insert/update/delete/response metadata 写入轻量 SQLite。Edged 启动后主动查询本地 pod 列表，把结果作为 `SET` 送给 lite kubelet，并经 CRI 管理容器。断网时需要远端的 ConfigMap、Secret 等查询不再发往云端，而会查询本地 DB；只有此前已经同步且仍适用的数据可用。

**边界。已证实事实**：这个 SQLite 是每个边缘节点的 metadata/cache 与恢复输入，不是云端 Kubernetes 状态的完整副本，不提供任意 list-watch、跨节点事务或离线创建所有控制面资源的承诺。继续运行主要指现有本地工作负载和设备逻辑可以不依赖持续云连接；远端依赖、首次拉取、调度、集中审批和新凭证仍可能失败。

**已证实事实**：Device v1beta1 在云端以 `spec.properties[].desired` 表达期望值，以 `status.twins[].reported` 表达设备报告值，并用 `observedDesired` 表示 Mapper 已观察到的期望。EdgeCore DeviceTwin 的消息与 SQLite 模型使用 `expected/actual` 以及 cloud/edge version；Mapper 才负责按 desired 操作物理设备并收集 reported，DeviceTwin 不是执行器，也不是不可变事务日志。

**已证实事实**：CloudHub 对需要确认的资源消息等待 EdgeCore 在 MetaManager 成功存储后返回 ACK；未确认消息会重试。ObjectSync/ClusterObjectSync 记录已成功下发的资源版本，SyncController 周期比较云端当前版本并重新生成事件。NO-ACK 消息则允许丢失；DeviceTwin 断网时会跳过发云消息，重连时请求 membership detail，但这不是所有离线 twin update 的持久 outbox，且设备消息成功点仍有源码 TODO。因此重连意味着继续同步与协调，不等于所有消息 exactly-once 或自动解决所有语义冲突。

**基于证据的推断**：Agent 平台可以沿同一边界把云端定义为 policy/model/tool bundle 的签发者，把边缘定义为受租约约束的执行者。每个离线动作带 `lease_id`、策略版本、模型/工具 digest、设备身份、风险级别、输入摘要和本地单调序号；本地 append-only audit/outbox 保存结果，重连后先对账再发布，不能让迟到结果直接覆盖新状态。
...
本文于 **2026-07-22** 选择稳定 release **KubeEdge v1.20.0**。固定 tag 为 [`v1.20.0`](https://github.com/kubeedge/kubeedge/releases/tag/v1.20.0)，`git ls-remote` 与本地精确 fetch 均解析到完整提交 [`bae61505d4919c404665f70347ad0646aaa98958`](https://github.com/kubeedge/kubeedge/tree/bae61505d4919c404665f70347ad0646aaa98958)；该提交时间为 2025-01-21 12:40:26 UTC，提交信息为更新 v1.20.0 版本。官方 release blog 记录 v1.20 于 2025-01-21 发布。

**版本一致性。已证实事实**：架构与组件语义只采用 [`release-1-20.docs.kubeedge.io`](https://release-1-20.docs.kubeedge.io/docs/) 下的 v1.20 文档，并用同一 `v1.20.0` commit 的源码核对。官网默认 `kubeedge.io/docs` 在访问日包含晚于 v1.20 的页面与组件变化，本文不使用那些后续语义；也不把 `release-1.20` 分支之后的修复倒灌到 tag 快照。
...
文字等价描述：云端 Operator 通过 Kubernetes API 修改 pod 或 Device CRD；CloudCore controllers 产生面向节点的期望状态，CloudHub 经 WebSocket/QUIC 连接把消息送到 EdgeHub。EdgeCore 内 MetaManager 持久化节点局部 metadata，Edged 经 CRI 运行容器；DeviceTwin 保存 expected/actual，EventBus 或 DMI 边界把它交给 Mapper，Mapper 才与物理设备交互。断网时云边虚线被切断，已运行应用只可依据未过期权限租约和本地 cache 执行，并写本地 outbox；重连后先上报和协调，不直接把离线结果当成云端新真相。
...
3. **EdgeHub 接收。** EdgeHub 从 WebSocket/QUIC 收取消息并按 group 路由到 MetaManager、DeviceTwin 或 EventBus。它也把边缘状态反向发送。读、写或 heartbeat 错误会触发断开；连接状态广播给 meta/twin/bus group，随后按 heartbeat 配置重连。
...
8. **重连协调。** EdgeHub 新建连接并广播 connected；需要 ACK 的资源继续按版本同步，MetaManager 周期上报 pod status。DeviceTwin 从 disconnected 变为 connected 时会请求 membership detail；后续显式 twin sync 或新的设备报告可更新云端 reported/observedDesired，但断网期间被跳过的所有发云消息不会自动变成完整重放日志。重复和迟到消息必须按资源/twin/业务版本判定，不能按到达顺序覆盖。
...
**个人分析**：每次本地动作先写 append-only audit/outbox，再调用工具或设备，随后追加 receipt，不原地改写历史。记录至少包含单调序号、boot ID、wall clock、monotonic elapsed、输入/输出 hash、策略判定、模型/工具版本、设备证书指纹、效果分类与操作者。这个 outbox 是应用设计，不应假装成 MetaManager cache 或 DeviceTwin twin 表。

## 关键源码导读

固定快照：[`kubeedge/kubeedge@bae61505d4919c404665f70347ad0646aaa98958`](https://github.com/kubeedge/kubeedge/tree/bae61505d4919c404665f70347ad0646aaa98958)，tag `v1.20.0`。下表只解释该快照公开行为。
...
**安全。个人分析**：设备身份应独立于 node identity，最好由 TPM/secure element 保护私钥；Mapper 建立会话时验证设备证书或硬件标识，并把设备指纹写入每条 command receipt。工具凭证按动作即时派生、短期有效且不进入模型 prompt、SQLite metadata 或普通日志。断网撤销天然有传播延迟，所以高风险证书和租约必须更短，并配本地 denylist/epoch。
...
**可观测性。基于证据的推断**：KubeEdge 的连接、resource version、pod status 和 twin version 只能说明基础同步状态。Agent 还要记录 `lease_id/policy/model/tool/security_epoch`、offline duration、risk class、operation ID、device receipt、base/result version、clock uncertainty 和 human takeover reason，才能解释为什么一个离线动作被允许以及重连后如何裁决。
