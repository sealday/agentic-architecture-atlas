...
- **当前发布基线：** 2026-07-24 成功部署的内容基线为 commit [`9eda726`](https://github.com/sealday/agentic-architecture-atlas/commit/9eda726df8c7ea13d758e2df89be2558129f822e)，发布证据为 [GitHub Actions run 30069822760](https://github.com/sealday/agentic-architecture-atlas/actions/runs/30069822760)；[`/patterns`](https://sealday.github.io/agentic-architecture-atlas/patterns) 已按通用设计、集成、可靠性、数据和迁移五个公共组展示，并保留独立的 Agent 控制与协作 wrapper。72 个 Pattern topic 均有唯一注册分组，计划主题只显示标题而不生成站内链接。随后仅承载本记录的 metadata commit 不改变该内容基线。
