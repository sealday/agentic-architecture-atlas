...
- 已发布知识页的 `adjacent_topics` 是互惠关系；A 声明 B 时，B 必须声明 A。它仍不参与
  `depends_on` 的依赖深度或环检查。
- 三个 registry 是验证、生成和运行时投影的必需输入；文件缺失、不可读或 schema 非法时必须
  fail closed，不能退回硬编码默认值或跳过检查。
- Node.js 中从 `import.meta.url` 派生文件系统路径时，统一先用 `fileURLToPath(new URL(...))`
  转换，再传给 `path`、`fs`、内容扫描器或生成器。
...
迁移组首批正式 planned topic 固定为：

| Topic | 标题 |
| --- | --- |
| `PAT-MIG-01` | Strangler Fig 渐进迁移 |
| `PAT-MIG-02` | Branch by Abstraction |
| `PAT-MIG-03` | Expand/Contract |

backlog parser、topic prefix registry 和 pattern group registry 必须同时识别 `PAT-MIG`。五个公开
通用组 `general-design`、`integration`、`reliability`、`data`、`migration` 都必须至少有一个
topic；planned topic 只显示无链接卡片，不生成占位文章或 404。`agent-control` 是独立人工入口，
不受通用组非空约束。
...
哪些文章采用 `quarterly-version-sensitive` 由作者和审查者依据事实易变性显式声明，不从
`checked_at`、`link_policy` 或一次 transport 探测推断策略或版本变化。承担 definition、method、
runtime-fact、case-evidence 或 implementation 的 source 仍必须有非空 `version`；缺失版本是
独立验证错误，而不是“来源已变化”的替代信号。discovery/learning/navigation-only 的索引来源
不能帮助文章满足事实证据门槛。
...
为满足互惠约束，`QA-01` 还必须反向列出 `REL-02`，`STY-00` 还必须反向列出 `MOD-02`。最终
关系为 `PR-01` ↔ `STY-00`、`QA-01` ↔ `MTH-03`、`REL-02` ↔ `QA-01`、
`MOD-02` ↔ `STY-00`。每篇还必须链接所属入口和至少一个现有案例。
...
- 至少引用两个彼此独立的来源，其中至少一个可以承担该文章事实门槛的 primary 或 first-party
  来源；同一组织的镜像页或同一项目的多个页面不算独立来源；
- 在“学习问题”中提出 3–5 个可由本文回答的问题；
- 包含原创 Mermaid 图或原创决策表，以及一个从输入、决策到结果完整闭环的演练；
- 明确给出反例、失败模式和“不应使用”的条件；
...
- 每页分别留下 editorial、fact、copyright、render 四项审查证据，不能用集合级结论代替逐页审查。

`PR-01` 新增的 Parnas DOI source 使用
`license_family_id: "doi:10.1145/361598.361623"`。其 `transport_locator`、`link_policy` 和
expected-final 字段只按完整 live link 审计的实际结果填写，不能从 DOI URL 形式预判为 stable。
新增来源后运行现有全量原子刷新流程，审阅完整结果并提交完整 cache；不实现仅合并两条结果的旁路。
...
- 五个公开通用分组必须非空；“计划中”只表示组内尚无已发布页，不制造空文章；
...
- 模块初始化时先执行完整 `assertCaseSeriesRegistry()`，再校验 catalog；缺 registry、未知字段、
  重复 ID/order、空 label 或非法展示标志都拒绝渲染；
...
`data/topic-relations.json` 的 override 管道完整接受 `dependencies`、`adjacent_topics`、
`related_cases`、`related_questions` 四个字段，但只允许 planned topic 写入。已发布 topic 的
front matter 是四类关系的唯一写入源；只要已发布 topic 仍有 override 就报错，即使两份值完全
相同，也不做静默合并或“相同即接受”。
...
review CLI 必须从规范 content、source ledger 和 review-policy registry 构建输入，不接受隐式空
数组。source 或 document 数量为零时报告无效。JSON 和 Markdown 均输出非空 `counts`：
`new_sources`、`rechecked_sources`、`orphan_sources`、`due_documents`、
`approaching_due_documents`，并明确列出通过/失败 gates。

### 12.3 季度到期判断

只有显式声明 `quarterly-version-sensitive` 且
`source_cutoff` 加三个日历月不晚于 `as_of` 时，已发布文章进入 due。`checked_at` 和
`link_policy` 只服务来源核对与链接健康，不能推断版本已变化或提前触发内容到期。

以下问题仍 fail closed，但作为独立一致性错误报告，不伪装成 due reason：

- 承担事实角色的 source 缺失非空 `version`；
- ledger document `reviewed_at` 早于文章 `source_cutoff`。

报告为每篇 due 文章列出：

- article slug、policy、`analyzed_at`、`source_cutoff` 和 ledger `reviewed_at`；
- 到期日和唯一到期原因 `interval-elapsed`；
...
validator CLI、`buildCaseCatalog`、内容平台生成入口和直接单元测试必须通过同一个 registry loader
取得三个 registry；缺失或不可读都产生确定性错误，不能因调用路径不同跳过。CLI 和 ESM 测试先用
`fileURLToPath(new URL(...))` 得到项目根和内容根。测试若直接调用 manifest/catalog builder，
必须传入真实或最小合法 registry 与非空 `primarySourcesByFile`；每个 published fixture 都要有
对应 primary-source map 项。
...
- review policy：
  `content/...mdx: factual source "source-id" is missing version`；
- due report：
...
- 任一必需 registry 缺失、不可读或 schema 非法；
- 已发布 pattern 没有且只有一个分组；
- case 使用未知 series；
- 关系目标不存在、类型错误或不能解析到已发布页面；
- 可见正文缺少关系链接；
- 日期不是合法日历日期；
- 已声明 review policy 未注册；
- 到期文章仍参与正式发布验证。
...
- 六篇文章逐页验证两个独立来源、3–5 个学习问题、原创图/决策表、完整演练、反例/不用条件，
  以及 editorial/fact/copyright/render 四项审查证据；
...
- `/patterns` 保持不变；
- 五个公开通用组均非空，三个 `PAT-MIG-*` planned topic 不产生 404；
...
- 18 个现有 slug 和 `catalog_order` 不变；快照只保存在 regression fixture，运行时和分类逻辑
  不导入该 fixture；
...
- adjacent relation 必须互惠，缺少反向边失败；
...
- planned override 四字段完整投影；published topic 存在任意 override 时失败。

### 15.5 周期复核

- 注入式时钟覆盖月末、闰日和“加三个日历月”边界；
- `registered_at` 与 `checked_at` 分别进入新增/重新核对列表；
- `checked_at` 或 `link_policy` 变化不会提前 due；
- discovery-only community index 不触发版本敏感事实门槛；
- 缺失版本、坏日期、未知 policy、reviewed/cutoff 不一致和到期文章失败，且前四者不冒充
  `interval-elapsed`；
- approaching-due 只报告不失败；
- workflow 保持只读权限、固定 action SHA，并总是上传报告；
- review CLI 拒绝零 source 或零 document 的空报告，JSON/Markdown 含非空 counts 与 gate 结果；
- 默认 `verify` 不发网络请求。
...
- 缺失 registry 的 validator CLI、manifest builder、case catalog builder 和生成入口全部失败；
- manifest 测试为每个 published fixture 提供真实 `primarySourcesByFile` 映射。
...
每个单元必须独立通过完整 verify、构建、独立审查、GitHub Pages 部署和线上 smoke 后再更新
backlog 与发布基线。leader 更新并提交 metadata 后，还必须有界轮询匹配该 metadata commit
`headSha` 的 Pages run、等待成功并再次 smoke，才算该单元结束。Actions 手工 dispatch 同样按
`headSha` 和触发时间有界定位 run，等待完成，下载并校验预期 artifact 与报告 counts/gates。
workflow 的 report build 与两个 artifact upload 步骤都使用 `if: always()`，确保前置 live
检查失败时仍留下诊断。任何单元失败时只回滚该单元，不回退已发布且验证通过的前置单元。

最终 cleaner 若改变文件，必须先提交 cleaner 变更，再重新执行完整 verify、独立 code review、
architecture review、Pages 等待和 smoke；审查证据必须指向 cleaner 后的 commit。G004 checkpoint
之前由 leader 再调用一次 `get_goal({})`，把工具返回的完整 JSON（不裁剪 objective、status、
budgets、usage 或其他字段）原样序列化并验证仍为 `active`，然后仅以该文件执行一次中间 aggregate
checkpoint；不得调用 `update_goal`，也不得提交手写字段子集。
...
- 删除硬编码 case manifest 后，完整性由内容发现集合、唯一 slug 和唯一 order 共同保证；原 18
  个 slug/order 迁入测试专用 regression fixture，不再作为分类或运行时真源。
...
- **歧义检查：** 六篇 fixture 已精确到 topic；method 与 modeling 分别有真实生产样板；
  `related_questions` 明确可选且与 `related_cases` 使用 OR 门禁；相邻关系明确为互惠；
  版本敏感由显式 policy 声明，季度到期只由日历区间触发。
