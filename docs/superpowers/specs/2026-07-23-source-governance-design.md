...
- `transport_locator` 是外链检查的网络目标。默认把 canonical URL 的 fragment 与 query 清空，
  同时规范化 scheme/host 为小写并移除默认端口；路径大小写和尾斜杠不自动改写。
- `citation_url` 保留正文实际引用的完整 query、fragment 和 GitHub `#Lx-Ly`，不得被 transport
  canonicalization 改写。
- `locator_aliases` 保存对象
  `{locator, transport_locator, expected_final_transport_locator, superseded_at}`。旧 citation 可以
...
- canonical URL、aliases 和 citation URL 经去 query/fragment 得到相同 transport base 时，只做
...
- `LicenseRef-Unknown` 与 `LicenseRef-All-Rights-Reserved` 禁止 adapted modes；
...
生成阶段只因 cache schema/coverage 不合法而停止；合法的 `review_status=stale` 仍进入 public
ledger 供页面明确展示。独立 `npm run check:links` 对 stale 或超过 120 天退出 1，因此总 verify
仍 fail closed。
...
- transport dedupe 还移除 query；citation 继续保留完整 query/fragment；
- 最终 transport 同时与 source 的 `expected_final_transport_locator` 和 previous
  `last_success.final_transport_locator` 比较；任一未审查变化均为 `redirect-changed`，stable 与
  floating 都不能自动接受；
...
- stable source identity、精确 citation 和 transport health 已分层；query/fragment 不制造重复
  source 或网络请求，alias/tombstone 保留历史。
