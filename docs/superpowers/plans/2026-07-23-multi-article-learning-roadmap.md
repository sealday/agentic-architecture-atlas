...
Numeric file prefixes lock the autogenerated sidebar order. The existing navigation contract reserves `sidebar_position: 5` for the learning-path overview, so the six stages use positions 6–11 and the four topics use positions 12–15. Published slugs remain semantic and contain no numeric prefix.
...
3. Assert main stages use `sidebar_position` 6 through 11.
4. Assert topics use `sidebar_position` 12 through 15.
5. Assert all common headings occur in order.
6. Assert topic articles also contain `## 当前已覆盖` and `## 后续待补`.
7. Assert the overview preserves `sidebar_position: 5`, references `/img/paths/software-architecture-learning-roadmap.png`, and contains the required Mermaid flowchart.
...
Use positions 7–11 and the corresponding slugs for the remaining stages. Keep `domains`, `agent_patterns`, `protocols`, `quality_attributes`, `tags`, and `official_sources` scoped to the article instead of copying the full overview list into every file.
...
Use `content_type: path`, `status: reviewed`, and positions 12–15. Use these slugs:
...
Confirm the image is clear, the Mermaid graph renders, the learning-path overview and ten articles appear in positions 5–15, adjacent links work, and no console errors appear.
